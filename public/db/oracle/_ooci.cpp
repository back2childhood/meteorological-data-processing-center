...
函数执行完成，但没有其他数据 OCI_ERROR                 -1  // maps to SQL_ERROR
函数执行错误 OCI_INVALID_HANDLE        -2  // maps to SQL_INVALID_HANDLE
传递给函数的参数为无效句柄，
                              // 或传回的句柄无效
OCI_NEED_DATA             99  // maps to SQL_NEED_DATA
需要应用程序提供运行时刻的数据 OCI_STILL_EXECUTING    -3123  // OCI would block
error 服务环境建立在非阻塞模式，
                              // OCI函数调用正在执行中
...
int oci_init(LOGINENV *env) {
  ...
  oci_ret =
      OCIEnvCreate(&env->envhp, OCI_DEFAULT, NULL, NULL, NULL, NULL, 0, NULL);
...
int oci_context_create(LOGINENV *env, OCI_CXT *cxt) {
  ...
  oci_ret = OCIHandleAlloc(env->envhp, (dvoid **)&cxt->errhp, OCI_HTYPE_ERROR,
                           (size_t)0, NULL);
  ...
  // 登录
  oci_ret = OCILogon(env->envhp, cxt->errhp, &cxt->svchp, (OraText *)env->user,
                     strlen(env->user), (OraText *)env->pass, strlen(env->pass),
                     (OraText *)env->tnsname, strlen(env->tnsname));
...
int oci_stmt_create(OCI_CXT *cxt, OCI_HANDLE *handle) {
  ...
  oci_ret = OCIHandleAlloc(cxt->envhp, (dvoid **)&handle->smthp, OCI_HTYPE_STMT,
                           (size_t)0, NULL);
...
void connection::err_report() {
  ...
  if (m_cxt.errhp != NULL) {
    if (OCIErrorGet(m_cxt.errhp, 1, NULL, &m_cda.rc, (OraText *)m_cda.message,
                    sizeof(m_cda.message), OCI_HTYPE_ERROR) == OCI_NO_DATA) {
      // 如果获取不到错误信息，就返回正确的
      memset(&m_cda, 0, sizeof(m_cda));
      return;
    }
...
int sqlstatement::prepare(const char *fmt, ...) {
  ...
  int oci_ret = OCIStmtPrepare(m_handle.smthp, m_handle.errhp, (OraText *)m_sql,
                               strlen(m_sql), OCI_NTV_SYNTAX, OCI_DEFAULT);
...
int sqlstatement::bindout(unsigned int position, char *value,
                          unsigned int len) {
  return OCIDefineByPos(m_handle.smthp, &m_handle.defhp, m_handle.errhp,
                        position, value, len + 1, SQLT_STR, NULL, NULL, NULL,
                        OCI_DEFAULT);
}
...
int sqlstatement::bindblob() {
  ...
  return OCIDefineByPos(m_handle.smthp, &m_handle.defhp, m_handle.errhp, 1,
                        (dvoid *)&m_lob, -1, SQLT_BLOB, NULL, NULL, NULL,
                        OCI_DEFAULT);
}
...
int sqlstatement::bindclob() {
  ...
  return OCIDefineByPos(m_handle.smthp, &m_handle.defhp, m_handle.errhp, 1,
                        (dvoid *)&m_lob, -1, SQLT_CLOB, NULL, NULL, NULL,
                        OCI_DEFAULT);
}
...
int sqlstatement::execute() {
  ...
  int oci_ret = OCIStmtExecute(m_handle.svchp, m_handle.smthp, m_handle.errhp,
                               m_sqltype, 0, NULL, NULL, mode);
  ...
  // 如果不是查询语句，就获取影响记录的行数
  if (m_sqltype == 1) {
    OCIAttrGet((CONST dvoid *)m_handle.smthp, OCI_HTYPE_STMT,
               (dvoid *)&m_cda.rpc, (ub4 *)0, OCI_ATTR_ROW_COUNT,
               m_handle.errhp);
...
int sqlstatement::next() {
  ...
  int oci_ret = OCIStmtFetch(m_handle.smthp, m_handle.errhp, 1, OCI_FETCH_NEXT,
                             OCI_DEFAULT);
  ...
  if (oci_ret != OCI_SUCCESS && oci_ret != OCI_SUCCESS_WITH_INFO) {
    ...
    if (m_cda.rc != 1405 && m_cda.rc != 1406) {
      ...
      /*
      if ( (m_cda.rc!=0) && (m_cda.rc!=1403) && (m_cda.rc!=1405) &&
      (m_cda.rc!=1406) )
      {
        m_conn->disconnect();
      }
  ...
  // 获取影响记录的行数据
  OCIAttrGet((CONST dvoid *)m_handle.smthp, OCI_HTYPE_STMT, (dvoid *)&m_cda.rpc,
             (ub4 *)0, OCI_ATTR_ROW_COUNT, m_handle.errhp);
...
void sqlstatement::err_report() {
  ...
  if (m_handle.errhp != NULL) {
    if (OCIErrorGet(m_handle.errhp, 1, NULL, &m_cda.rc,
                    (OraText *)m_cda.message, sizeof(m_cda.message),
...
int sqlstatement::alloclob() {
  ...
  return OCIDescriptorAlloc((dvoid *)m_handle.envhp, (dvoid **)&m_lob,
...
int sqlstatement::filetolob(FILE *fp) {
  ...
  if (remainder == 0) {
    ...
    if ((retval =
             OCILobWrite(m_handle.svchp, m_handle.errhp, m_lob, &amtp, offset,
                         (dvoid *)bufp, (ub4)nbytes, OCI_ONE_PIECE, (dvoid *)0,
                         (sb4(*)(dvoid *, dvoid *, ub4 *, ub1 *))0, (ub2)0,
    ...
    // more than one piece
    if (OCILobWrite(m_handle.svchp, m_handle.errhp, m_lob, &amtp, offset,
                    (dvoid *)bufp, (ub4)LOBMAXBUFLEN, OCI_FIRST_PIECE,
    ...
    do {
      ...
      retval = OCILobWrite(m_handle.svchp, m_handle.errhp, m_lob, &amtp, offset,
                           (dvoid *)bufp, (ub4)nbytes, piece, (dvoid *)0,
                           (sb4(*)(dvoid *, dvoid *, ub4 *, ub1 *))0, (ub2)0,
                           (ub1)SQLCS_IMPLICIT);
      remainder -= nbytes;
...
int sqlstatement::lobtofile(FILE *fp) {
  ...
  retval = OCILobRead(
      m_handle.svchp, m_handle.errhp, m_lob, &amtp, offset, (dvoid *)bufp,
  ...
  case OCI_NEED_DATA: /* there are 2 or more pieces */
    ...
    while (1) {
      ...
      retval = OCILobRead(m_handle.svchp, m_handle.errhp, m_lob, &amtp, offset,
                          (dvoid *)bufp, (ub4)LOBMAXBUFLEN, (dvoid *)0,
