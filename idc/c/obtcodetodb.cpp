...
int main(int argc, char *argv[]) {
  ...
  if (argc != 5) {
    ...
    printf(
        "Example:/project/tools1/bin/procctl 120 /project/idc1/bin/obtcodetodb "
        "/project/idc/ini/stcode.ini \"127.0.0.1,root,mysqlpwd,mysql,3306\" "
        "utf8 /log/idc/obtcodetodb.log\n\n");

    printf("本程序用于把全国站点参数数据保存到数据库表中，如果站点不存在则插入"
           "，站点已存在则更新。\n");
  ...
  stmtupt.prepare("update T_ZHOBTCODE set "
                  "cityname=:1,provname=:2,lat=:3*100,lon=:4*100,height=:5*10,"
                  "upttime=now() where obtid=:6");
